...
export function initGame() {
    ...
    const imageUrl = 'https://raw.githubusercontent.com/seferbreak/-/main/pistol_transparent.png';
    ...
    const legGeo = new THREE.CylinderGeometry(0.15, 0.15, 3.4, 16);
    legGeo.translate(0, -1.7, 0); 
    const legMat = new THREE.MeshStandardMaterial({color: 0x11161a, roughness: 0.9});
    ...
    yawObject.add(leftLeg);
    yawObject.add(rightLeg);
    ...
    function animate() {
        ...
        if (isMoving) {
            ...
            leftLeg.rotation.x = Math.sin(bobTimer) * 0.6; 
            rightLeg.rotation.x = Math.sin(bobTimer + Math.PI) * 0.6;
            ...
            leftLeg.rotation.x += (0 - leftLeg.rotation.x) * 0.1; 
            rightLeg.rotation.x += (0 - rightLeg.rotation.x) * 0.1;
        }
            ...
            if (m.userData.state === 'idle') {
                ...
                if (m.userData.isJumping) {
                    ...
                    if (time - (m.userData.lastAttack || 0) > 0.4) {
                        ...
                        m.userData.jumpTargetX = yawObject.position.x; m.userData.jumpTargetZ = yawObject.position.z;
